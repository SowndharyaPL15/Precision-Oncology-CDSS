...
from app.models.efficientnet_model import build_efficientnet_b0, prepare_for_fine_tuning
from app.core.training_engine import TrainingEngine

def main():
    parser = argparse.ArgumentParser(description="Train EfficientNetB0 on Lung Cancer Dataset")
    parser.add_argument("--mode", type=str, default="verification", choices=["verification", "final"])
    parser.add_argument("--epochs", type=int, default=None)
    args = parser.parse_args()

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    dataset_path = os.path.join(project_root, "datasets", "lungs")
    
    config = PreprocessingConfig()
    builder = DataPipelineBuilder(
        config=config,
        dataset_name="Lung Cancer (LC25000)",
        root_dir=dataset_path
    )

    all_paths, all_labels = builder._collect_files_from_class_dirs(dataset_path)
    all_labels = np.array(all_labels)
    num_classes = len(builder.class_names)

    train_paths, temp_paths, train_labels, temp_labels = train_test_split(
        all_paths, all_labels, test_size=0.30, stratify=all_labels, random_state=config.seed
    )
    val_paths, test_paths, val_labels, test_labels = train_test_split(
        temp_paths, temp_labels, test_size=0.50, stratify=temp_labels, random_state=config.seed
    )
    ...
    if args.mode == "verification":
        ...
        _, val_paths, _, val_labels = train_test_split(
            val_paths, val_labels, test_size=0.10, stratify=val_labels, random_state=config.seed
        )
